...
class NeighbourChatScreenController extends GetxController {
  ...
  StreamController<List<ViewConversationNeighbours>>
      conversationneighboursstreamController =
      StreamController<List<ViewConversationNeighbours>>();
  ...
  Future<void> _initiatePusherSocketForMessaging() async {
    pusher = PusherClient(
        YOUR_APP_KEY,
        PusherOptions(
          host: 'http://192.168.2.45:8080',
          cluster: 'ap2',
          auth: PusherAuth(
            'http://192.168.2.45:8080',
            headers: {
...
//
  ...
  Future<List<ViewConversationNeighbours>> ViewConversationNeighboursApi(
      {required int chatroomid, required String token}) async {
    print(token);

    final response = await Http.get(
      Uri.parse(Api.viewconversationsneighbours + "/" + chatroomid.toString()),
